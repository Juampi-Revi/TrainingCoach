...
export default function SessionCompletadaPage() {
  ...
  const plannedBlocks = session.blocks.filter((b) => (b.steps?.length ?? 0) > 0);
    ...
    <div style={{ minHeight: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginTop: 16 }}>
          <CompletadaStatCard label="VOLUMEN" value={Math.round(totalVol).toLocaleString("es")} unit="kg" />
          <CompletadaStatCard label="SERIES" value={`${totalSets}${targetSets > 0 ? `/${targetSets}` : ""}`} />
          <CompletadaStatCard label={isPartial ? "ESTADO" : "EJERCICIOS"} value={isPartial ? "PARCIAL" : String(workExercises.length)} />
        </div>
        <CompletadaHeroExtras
          session={session}
          totalSets={totalSets}
      ...
      <div style={{ flex: 1, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
        ...
        <CompletadaEnergyCard energy={energy} onChange={setEnergy} />

        <CompletadaHighlights highlights={highlights} />
      ...
      <div style={{ padding: "10px 16px 28px", borderTop: "1px solid var(--line)", display: "flex", gap: 8, background: "var(--bg)" }}>
        ...
        <Button size="lg" style={{ flex: 1.4 }} disabled={saving} onClick={save}>
          ...
          {!saving && <Icon name="check" size={14} color="var(--bg)" />}
        </Button>
      </div>
    </div>
  );
