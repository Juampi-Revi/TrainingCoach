  ...
  return withHandler(async () => {
    ...
    const [healthEntries, foodCount, sessions, latestMetric] = await Promise.all([
      ...
      prisma.foodLogEntry.count({
        where: { clientUserId: auth.user.sub, loggedAt: { gte: start, lt: endExclusive } },
      }),
      prisma.workoutSession.findMany({
        where: { clientUserId: auth.user.sub, performedAt: { gte: start, lt: endExclusive }, status: { not: "discarded" } },
        select: { status: true },
      }),
      prisma.bodyMetricEntry.findFirst({
        where: { clientUserId: auth.user.sub, weightKg: { not: null } },
